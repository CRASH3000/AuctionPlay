...
async def update_profile(update_req: ProfileUpdateRequest = Depends(ProfileUpdateRequest.as_form),
...
async def update_avatar(avatar_req: UserSetAvatarRequest = Depends(UserSetAvatarRequest.as_form),
                        current_user: User = Depends(get_current_user),
                        session: AsyncSession = Depends(get_session)):
