...
async def create_comment(comment: CommentCreate,
    ...
    result = await session.execute(select(Comment).where(Comment.post_id == literal(comment.post_id),
...
async def get_comments(post_id: int = Query(..., alias="id"),
                       session: AsyncSession = Depends(get_session)):
    ...
    for comment in comments:
        ...
        if comment.user_id is not None:
            result = await session.execute(select(User).where(User.id == literal(comment.user_id)))
...
async def get_max_bid(post_id: int, session: AsyncSession = Depends(get_session)):
    ...
    result = await session.execute(select(User).where(User.id == literal(max_comment.user_id)))
