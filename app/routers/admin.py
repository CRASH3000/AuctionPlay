...
@router.delete("/users/{user_id}", summary="Удаление пользователя по ID", tags=["Admin"])
...
@router.patch("/users/{user_id}/role", summary="Изменение роли пользователя по ID", tags=["Admin"])
